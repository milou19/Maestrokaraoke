...
function Settings() {
  ...
  const [mobilePhoneMode, setMobilePhoneMode] = useSettingValue(
    MobilePhoneModeSetting
  );

  const [camera, setCamera] = useState<null | boolean>(
    CameraManager.getPermissionStatus()
  );
  useEffect(() => {
  ...
  const enableCamera = () => {
    ...
    CameraManager.requestPermissions().then(() =>
  ...
  return (
    <MenuWithLogo>
      <Helmet>
        <title>
          Settings | maestrokaraoke.vercel.app - Free Online Karaoke Party Game
        </title>
      </Helmet>
      <h1>Settings</h1>
      <Switcher
        {...register("graphics-level", () =>
          setGraphicLevel(nextValue(GraphicsLevel, graphicLevel))
        )}
        label="Graphics"
        value={graphicLevel.toUpperCase()}
      />
      <Switcher
        {...register("fps-count-level", () =>
          setFpsCount(nextValue(FpsCount, fpsCount))
        )}
        label="FPS Count"
        value={fpsCount}
      />
      ...
      <div className="flex flex-col">
        ...
        <Typography className="text-sm">
          If the sound is not synchronised with the lyrics, use this to
          compensate it.
        </Typography>
      </div>
      <hr />
      <Switcher
        {...register("camera-access", () =>
          camera ? CameraManager.disable() : enableCamera()
        )}
      ...
      <hr />
      <MenuButton
        {...register("remote-mics-settings", () =>
          navigate("settings/remote-mics/")
        )}
        size="small"
      >
      ...
      <MenuButton
        {...register("setup-mics-button", () => navigate("select-input/"))}
        size="small"
      >
        Setup Microphones
      </MenuButton>
      <MenuButton
        {...register("manage-songs-button", () => navigate("manage-songs/"))}
        size="small"
      >
        Manage Songs
      </MenuButton>
      <MenuButton {...register("back-button", goBack)}>
        Return To Main Menu
      </MenuButton>
    </MenuWithLogo>
  );
