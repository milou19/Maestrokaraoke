...
export default function Edit() {
  ...
  return (
    ...
    >
      <Helmet>
        <title>
          Edit Song | maestrokaraoke.vercel.app - Free Online Karaoke Party Game
        </title>
      </Helmet>
      <TopBar>
        <Link to="edit/list/">
            ...
            <IconButton
              ...
              onClick={async () => {
                ...
                if (proceed) {
                  ...
                  if (
                    shareSongs &&
                    data.some((songInIndex) => songInIndex.id === song.data!.id)
                  ) {
        ...
        <abbr title={song.data.lastUpdate}>
          Last updated:{" "}
          <b>
            {song.data.lastUpdate ? dayjs(song.data.lastUpdate).fromNow() : "-"}
          </b>
        </abbr>
      </TopBar>
