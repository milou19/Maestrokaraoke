...
export default function Setlists() {
  ...
  const selectedSongs =
    setlists.find((setlist) => setlist.name === selectedSetlist)?.songList ||
    [];
  const onSelect = (selected: number[]) => {
    if (selected.length > MAX_SONGS_PER_SETLIST) {
      alert(
        `You can only select up to ${MAX_SONGS_PER_SETLIST} songs per setlist.`
      );
      return;
    }
    setSetlists((prevSetlists) =>
      prevSetlists.map((setlist) =>
        setlist.name === selectedSetlist
          ? { ...setlist, songList: selected }
          : setlist
      )
    );
  };
  ...
  return (
    <>
      <Helmet>
        <title>
          Song List | maestrokaraoke.vercel.app - Free Online Karaoke Party Game
        </title>
      </Helmet>
      <NoPrerender>
        <Container>
            ...
            <Grid item xs={12}>
              <div className="flex flex-col items-stretch gap-4">
                <p className="leading-6">
                  This feature allows you to create a custom game link that
                  includes only a selected subset of songs. It&#39;s useful for
                  contests or situations where you want to limit which songs are
                  visible. Note that{" "}
                  <strong>locally added songs cannot be included</strong>.
                  Additionally, if you add or remove a song,{" "}
                  <strong>the setlist link will change</strong> — so it&#39;s
                  ...
                  to generate a consistent short link and update it whenever you
                  modify the setlist. Each setlist can contain up to{" "}
                  {MAX_SONGS_PER_SETLIST} songs.
                </p>
                <p>
                  You can also make a setlist non-editable, which prevents users
                  from adding additional songs to it.
                </p>
                  ...
                  return (
                    ...
                    >
                      ...
                      <Button
                        data-test={
                          setlist.isEditable
                            ? "make-setlist-not-editable"
                            : "make-setlist-editable"
                        }
                        onClick={() =>
                          setSetlists(
                            setlists.map((s) =>
                              s.name === setlist.name
                                ? { ...s, isEditable: !s.isEditable }
                                : s
                            )
                          )
                        }
                      ...
                      <Button
                        ...
                        onClick={() => {
                          const confirmed = confirm(
                            `Are you sure you want to delete the setlist "${setlist.name}"?`
                          );
                          if (confirmed) {
                            setSetlists((prevSetlists) =>
                              prevSetlists.filter(
                                (s) => s.name !== setlist.name
                              )
                            );
                      ...
                      <Button
                        ...
                        onClick={() => {
                          ...
                          url.searchParams.set(
                            "setlist",
                            encodeSongList(setlist.songList, setlist.isEditable)
                          );
